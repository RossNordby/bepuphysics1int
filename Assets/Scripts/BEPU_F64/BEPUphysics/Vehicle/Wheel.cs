...
namespace BEPUphysics.Vehicle
{
    ...
    public class Wheel
    {
        ...
        internal Vector3 localForwardDirection = new Vector3(Fix32.Zero, Fix32.Zero, Fix32.MinusOne);
        ...
        internal void PreStep(Fix32 dt)
        {
            ...
            else
            {
                ...
                suspension.accumulatedImpulse = Fix32.Zero;
                slidingFriction.accumulatedImpulse = Fix32.Zero;
                drivingMotor.accumulatedImpulse = Fix32.Zero;
                brake.accumulatedImpulse = Fix32.Zero;
            }
        }
        ...
        internal bool ApplyImpulse()
        {
            ...
            if (suspension.isActive)
            {
                if (++suspension.solverSettings.currentIterations <= suspension.solverSettings.maximumIterationCount)
                    if (suspension.ApplyImpulse().Abs() < suspension.solverSettings.minimumImpulse)
                    {
            ...
            if (slidingFriction.isActive)
            {
                if (++slidingFriction.solverSettings.currentIterations <= suspension.solverSettings.maximumIterationCount)
                    if (slidingFriction.ApplyImpulse().Abs() < slidingFriction.solverSettings.minimumImpulse)
                    {
            ...
            if (drivingMotor.isActive)
            {
                if (++drivingMotor.solverSettings.currentIterations <= suspension.solverSettings.maximumIterationCount)
                    if (drivingMotor.ApplyImpulse().Abs() < drivingMotor.solverSettings.minimumImpulse)
                    {
            ...
            if (brake.isActive)
            {
                if (++brake.solverSettings.currentIterations <= suspension.solverSettings.maximumIterationCount)
                    if (brake.ApplyImpulse().Abs() < brake.solverSettings.minimumImpulse)
                    {
        ...
        internal void UpdateAtEndOfFrame(Fix32 dt)
        {
            shape.UpdateWorldTransform();
        }

        internal void UpdateAtEndOfUpdate(Fix32 dt)
        {
            shape.UpdateSpin(dt);
        }

        internal void UpdateDuringForces(Fix32 dt)
        {
