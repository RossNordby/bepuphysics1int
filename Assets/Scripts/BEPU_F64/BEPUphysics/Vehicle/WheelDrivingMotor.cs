...
namespace BEPUphysics.Vehicle
{
    ...
    public class WheelDrivingMotor : ISolverSettings
    {
        ...
        public Fix32 GripFriction
        {
            ...
            set { gripFriction = MathHelper.Max(value, Fix32.Zero); }
        }
        ...
        public Fix32 RelativeVelocity
        {
            get
            {
                Fix32 velocity = Fix32.Zero;
                if (vehicleEntity != null)
                    velocity = velocity .Add (vehicleEntity.linearVelocity.X .Mul (linearAX) .Add (vehicleEntity.linearVelocity.Y .Mul (linearAY)) .Add (vehicleEntity.linearVelocity.Z .Mul (linearAZ)) .Add
        ...
        internal Fix32 ApplyImpulse()
        {
            ...
            Fix32 lambda = (RelativeVelocity
                            .Sub (targetSpeed)) //Add in the extra goal speed
            ...
            if (targetSpeed > Fix32.Zero)
                accumulatedImpulse = MathHelper.Clamp(accumulatedImpulse, Fix32.Zero, maxMotorForceDt); //MathHelper.Min(MathHelper.Max(accumulatedImpulse, 0), myMaxMotorForceDt);
            else if (targetSpeed < Fix32.Zero)
                accumulatedImpulse = MathHelper.Clamp(accumulatedImpulse, maxMotorForceDt, Fix32.Zero); //MathHelper.Max(MathHelper.Min(accumulatedImpulse, 0), myMaxMotorForceDt);
            else
