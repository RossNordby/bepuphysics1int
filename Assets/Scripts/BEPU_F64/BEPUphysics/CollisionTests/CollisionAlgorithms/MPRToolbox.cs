...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public static class MPRToolbox
    {
        ...
        private static Fix32 surfaceEpsilon = 1e-7m.ToFix32();
        /// <summary>
        ...
        private static Fix32 depthRefinementEpsilon = 1e-4m.ToFix32();
        /// <summary>
        ...
        private static Fix32 rayCastSurfaceEpsilon = 1e-9m.ToFix32();
        /// <summary>
        ...
        public static void LocalSurfaceCast(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 direction, out Fix32 t, out Vector3 normal)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon)
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon.Mul(F64.C0p01))
                    Vector3.Divide(ref direction, rayLengthSquared.Sqrt(), out normal);
                else
                    normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot.Sub(dot) < surfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Divide(ref n, lengthSquared.Sqrt(), out normal);
        ...
        static bool VerifySimplex(ref Vector3 v0, ref Vector3 v1, ref Vector3 v2, ref Vector3 v3, ref Vector3 direction)
        {
            ...
            return (planeProduct1 <= Fix32.Zero && planeProduct2 <= Fix32.Zero && planeProduct3 <= Fix32.Zero) ||
                (planeProduct1 >= Fix32.Zero && planeProduct2 >= Fix32.Zero && planeProduct3 >= Fix32.Zero);
        }
        ...
        public static bool GetContact(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform transformA, ref RigidTransform transformB, ref Vector3 penetrationAxis, out ContactData contact)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(shapeA, shapeB, ref localTransformB))
            {
                ...
                {
                    Vector3.Divide(ref penetrationAxis, lengthSquared.Sqrt(), out rayCastDirection);// (Vector3.Normalize(localDirection) + Vector3.Normalize(collidableB.worldTransform.Position - collidableA.worldTransform.Position)) / 2;
                ...
                if (lengthSquared > Toolbox.Epsilon)
                {
                    Vector3.Divide(ref localTransformB.Position, lengthSquared.Sqrt(), out rayCastDirection);
        ...
        public static bool Sweep(ConvexShape shapeA, ConvexShape shapeB, ref Vector3 sweepA, ref Vector3 sweepB, ref RigidTransform transformA, ref RigidTransform transformB, out RayHit hit)
        {
            ...
            if (rayLengthSquared > Toolbox.Epsilon.Mul(F64.C0p01))
            {
                ...
                sweepLength = sweepLength.Add( (shapeA.MaximumRadius.Add(shapeB.MaximumRadius)) .Div( rayLengthSquared.Sqrt() ));
            }
            else
            {
                rayLengthSquared = Fix32.Zero;
                sweepLength = Fix32.Zero;
            }
            //If the sweep direction is found to be negative, the ray can be thought of as pointing away from the shape.
            //Do not sweep backward.
            bool negativeLength = sweepLength < Fix32.Zero;
            if (negativeLength)
                sweepLength = Fix32.Zero;
            ...
            if (negativeLength)
            {
                ...
                hit.Location = hit.Location + (sweepA * hit.T);
                return true;
            }
        ...
        private static bool LocalSweepCast(ConvexShape shape, ConvexShape shapeB, Fix32 sweepLength, Fix32 rayLengthSquared, ref Vector3 localDirection, ref Vector3 sweep, ref RigidTransform localTransformB, out RayHit hit)
        {
            ...
            if (n.LengthSquared() < Toolbox.Epsilon.Mul(F64.C0p01))
            {
                ...
                if (rayLengthSquared > Toolbox.Epsilon.Mul(F64.C0p01))
                    Vector3.Divide(ref localDirection, rayLengthSquared.Sqrt(), out hit.Normal);
                else
                    hit.Normal = new Vector3();
            ...
            while (true)
            {
                ...
                if (supportDot.Sub(dot) < rayCastSurfaceEpsilon || count > MPRToolbox.InnerIterationLimit) // TODO: Could use a dynamic epsilon for possibly better behavior.
                {
                    ...
                    {
                        Vector3.Divide(ref n, lengthSquared.Sqrt(), out hit.Normal);
        ...
        internal static void GetLocalPosition(ConvexShape shapeA, ConvexShape shapeB, ref RigidTransform localTransformB, ref Vector3 minkowskiPosition, out Vector3 position)
        {
            ...
            while (true)
            {
                ...
                if (dot >= Fix32.Zero) //Dot v0 x v4 against v1, and dot it against 
                {
                    Vector3 pointToV2;
        ...
        public static bool RayCast(Ray ray, Fix32 maximumLength, ConvexShape shape, ref RigidTransform transform, out RayHit hit)
        {
            ...
            if (Toolbox.RayCastSphere(ref ray, ref transform.Position, shape.MaximumRadius, maximumLength, out sphereHit))
            {
                ...
                sphereHit.T = MathHelper.Max(sphereHit.T.Sub(F64.C0p1), Fix32.Zero);
                Vector3 offset;
            ...
            if (rayLengthSquared > Toolbox.Epsilon.Mul(F64.C0p01))
            {
                ...
                sweepLength = sweepLength.Div(rayLengthSquared);
                //Scale the sweep length by the margins.  Divide by the length to pull the margin into terms of the length of the ray.
                sweepLength = sweepLength.Add( shape.MaximumRadius.Div(rayLengthSquared.Sqrt()) );
            }
            else
            {
                rayLengthSquared = Fix32.Zero;
