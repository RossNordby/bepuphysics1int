    ...
    public class ContactPenetrationConstraint : SolverUpdateable
    {
        ...
        public Fix32 RelativeVelocity
        {
            get
            {
                ...
                if (entityA != null)
                {
                    lambda = entityA.linearVelocity.X .Mul (linearAX) .Add (entityA.linearVelocity.Y .Mul (linearAY)) .Add (entityA.linearVelocity.Z .Mul (linearAZ)) .Add
                             (entityA.angularVelocity.X .Mul (angularAX)) .Add (entityA.angularVelocity.Y .Mul (angularAY)) .Add (entityA.angularVelocity.Z .Mul (angularAZ));
                }
                if (entityB != null)
                {
                    lambda = lambda . Add ((entityB.linearVelocity.X).Neg() .Mul (linearAX) .Sub (entityB.linearVelocity.Y .Mul (linearAY)) .Sub (entityB.linearVelocity.Z .Mul (linearAZ)) .Add
                              (entityB.angularVelocity.X .Mul (angularBX)) .Add (entityB.angularVelocity.Y .Mul (angularBY)) .Add (entityB.angularVelocity.Z .Mul (angularBZ)));
                }
                return lambda;
            }
        }
        ...
        public override void Update(Fix32 dt)
        {
            ...
            //linearBX = (linearAX).Neg();
            //linearBY = (linearAY).Neg();
            //linearBZ = (linearAZ).Neg();
            ...
            velocityToImpulse = Fix32.MinusOne .Div (softness .Add (effectiveMassInverse));


            //Bounciness and bias (penetration correction)
            if (contact.PenetrationDepth >= Fix32.Zero)
            {
                bias = MathHelper.Min(
                    MathHelper.Max(Fix32.Zero, contact.PenetrationDepth .Sub (CollisionDetectionSettings.AllowedPenetration)) .Mul
                    (CollisionResponseSettings.PenetrationRecoveryStiffness) .Mul (updateRate),
                    CollisionResponseSettings.MaximumPenetrationRecoverySpeed);

                if (contactManifoldConstraint.materialInteraction.Bounciness > Fix32.Zero)
                {
                    //Target a velocity which includes a portion of the incident velocity.
                    Fix32 bounceVelocity = (RelativeVelocity).Neg();
                    if (bounceVelocity > Fix32.Zero)
                    {
                        var lowThreshold = CollisionResponseSettings.BouncinessVelocityThreshold .Mul (F64.C0p3);
                        var velocityFraction = MathHelper.Clamp((bounceVelocity .Sub (lowThreshold)) .Div (CollisionResponseSettings.BouncinessVelocityThreshold .Sub (lowThreshold) .Add (Toolbox.Epsilon)), Fix32.Zero, F64.C1);
        ...
        public override Fix32 SolveIteration()
        {
            ...
            accumulatedImpulse = MathHelper.Max(Fix32.Zero, accumulatedImpulse .Add (lambda));
