...
namespace BEPUphysics.Constraints
{
    ...
    public class JointBasis3D
    {
        ...
        {
            if (Vector3.Dot(primaryAxis, xAxis).Abs() > Toolbox.BigEpsilon ||
				Vector3.Dot(primaryAxis, yAxis).Abs() > Toolbox.BigEpsilon ||
				Vector3.Dot(xAxis, yAxis).Abs() > Toolbox.BigEpsilon)
        ...
        {
            if (Vector3.Dot(matrix.Backward, matrix.Right).Abs() > Toolbox.BigEpsilon ||
				Vector3.Dot(matrix.Backward, matrix.Up).Abs() > Toolbox.BigEpsilon ||
				Vector3.Dot(matrix.Right, matrix.Up).Abs() > Toolbox.BigEpsilon)
                throw new ArgumentException("The axes provided to the joint transform do not form an orthonormal basis.  Ensure that each axis is perpendicular to the other two.");
