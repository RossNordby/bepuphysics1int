    ...
    public class CharacterController : Updateable, IBeforeSolverUpdateable
    {
        ...
        public Vector3 Down
        {
            ...
            set
            {
                ...
                if (lengthSquared < Toolbox.Epsilon)
                    ...
                    Vector3.Divide(ref value, lengthSquared.Sqrt(), out value);
                Quaternion.GetQuaternionBetweenNormalizedVectors(ref Toolbox.DownVector, ref value, out orientation);
                Body.Orientation = orientation;
            }
        }

        Vector3 viewDirection = new Vector3(Fix32.Zero, Fix32.Zero, Fix32.MinusOne);
        ...
        public Vector3 ViewDirection
        {
            ...
            set
            {
                ...
                {
                    Vector3.Divide(ref value, lengthSquared.Sqrt(), out viewDirection);
                }
                else
                {
		...
		public CharacterController(
            ...
            Fix32? standingSpeed = null, Fix32? crouchingSpeed = null, Fix32? proneSpeed = null, Fix32? tractionForce = null, Fix32? slidingSpeed = null, Fix32? slidingForce = null, Fix32? airSpeed = null, Fix32? airForce = null,
            Fix32? jumpSpeed = null, Fix32? slidingJumpSpeed = null,
            Fix32? maximumGlueForce = null
			)
        {
			if (height == null)
				height = 1.7m.ToFix32Fast();
			if (crouchingHeight == null)
				crouchingHeight = (1.7m * .7m).ToFix32();
			if (proneHeight == null)
				proneHeight = (1.7m * 0.3m).ToFix32();
			if (radius == null)
				radius = 0.6m.ToFix32();
			if (margin == null)
				margin = 0.1m.ToFix32();
			...
			if (maximumTractionSlope == null)
				maximumTractionSlope = 0.8m.ToFix32();
			if (maximumSupportSlope == null)
				maximumSupportSlope = 1.3m.ToFix32();
			if (standingSpeed == null)
				standingSpeed = 8.ToFix32();
			if (crouchingSpeed == null)
				crouchingSpeed = 3.ToFix32();
			if (proneSpeed == null)
				proneSpeed = 1.5m.ToFix32();
			if (tractionForce == null)
				tractionForce = 1000.ToFix32();
			if (slidingSpeed == null)
				slidingSpeed = 6.ToFix32();
			if (slidingForce == null)
				slidingForce = 50.ToFix32();
			if (airSpeed == null)
				airSpeed = 1.ToFix32();
			if (airForce == null)
				airForce = 250.ToFix32();
			if (jumpSpeed == null)
				jumpSpeed = 4.5m.ToFix32();
			if (slidingJumpSpeed == null)
				slidingJumpSpeed = 3.ToFix32();
        ...
        void ExpandBoundingBox()
        {
            if (Body.ActivityInformation.IsActive)
            {
                ...
                expansion.X = (expansion.X).Abs();
                expansion.Y = (expansion.Y).Abs();
                expansion.Z = (expansion.Z).Abs();
                ...
                expansion.X = expansion.X .Add (horizontalExpansionAmount .Mul ((squaredDown.Y .Add (squaredDown.Z)).Sqrt()));
                expansion.Y = expansion.Y .Add (horizontalExpansionAmount .Mul ((squaredDown.X .Add (squaredDown.Z)).Sqrt()));
                expansion.Z = expansion.Z .Add (horizontalExpansionAmount .Mul ((squaredDown.X .Add (squaredDown.Y)).Sqrt()));
        ...
        void IBeforeSolverUpdateable.Update(Fix32 dt)
        {
            ...
            try
            {
                ...
                if (SupportFinder.HasSupport && !hadSupport && verticalVelocity < Fix32.Zero)
                {
                ...
                if (TryToJump)
                {
                    ...
                    if (SupportFinder.HasTraction)
                    {
                        ...
                        Fix32 currentDownVelocity = Vector3.Dot(Down, relativeVelocity);
                        //Target velocity is JumpSpeed.
                        Fix32 velocityChange = MathHelper.Max(jumpSpeed .Mul (currentDownVelocity), Fix32.Zero);
                    ...
                    else if (SupportFinder.HasSupport)
                    {
                        ...
                        //Target velocity is JumpSpeed.
                        Fix32 velocityChange = MathHelper.Max(slidingJumpSpeed .Sub (currentNormalVelocity), Fix32.Zero);
        ...
        SupportData TeleportToPosition(Vector3 newPosition, Fix32 dt)
        {
        ...
        void CorrectContacts()
        {
            ...
            Fix32 margin = Body.CollisionInformation.Shape.CollisionMargin;
            Fix32 minimumHeight = Body.Height .Mul (F64.C0p5) .Sub (margin);
            Fix32 coreRadius = Body.Radius .Sub (margin);
            Fix32 coreRadiusSquared = coreRadius .Mul (coreRadius);
            foreach (var pair in Body.CollisionInformation.Pairs)
            {
                foreach (var contactData in pair.Contacts)
                {
                    ...
                    if (dot > minimumHeight)
                    {
                        ...
                        Fix32 length = horizontalOffset.LengthSquared();
                        if (length > coreRadiusSquared)
                        {
                            //It's beyond the edge of the cylinder; clamp it.
                            Vector3.Multiply(ref horizontalOffset, coreRadius .Div (length.Sqrt()), out horizontalOffset);
                        }
                        ...
                        if (length > Toolbox.Epsilon)
                        {
                            ...
                            Vector3.Divide(ref offsetDirection, length.Sqrt(), out offsetDirection);
                        }
                        else
                        ...
                        if (dot > dotOriginal.Abs()) //if the new offsetDirection normal is less steep than the original slope...
                        {
                            //Then use it!
        ...
        {
            Body.LinearVelocity = Body.LinearVelocity + (velocityChange);
            var entityCollidable = supportData.SupportObject as EntityCollidable;
            if (entityCollidable != null)
            {
                if (entityCollidable.Entity.IsDynamic)
                {
                    ...
                    try
                    {
                        entityCollidable.Entity.LinearMomentum = entityCollidable.Entity.LinearMomentum + (change * Body.Mass.Neg());
                    }
                    finally
                    {
                        entityCollidable.Entity.Locker.Exit();
                    }
                    velocityChange = velocityChange + (change);
                }
            }
