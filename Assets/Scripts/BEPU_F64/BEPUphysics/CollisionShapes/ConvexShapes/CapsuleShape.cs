...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class CapsuleShape : ConvexShape
    {
        ...
        ///</summary>
        public Fix32 Radius { get { return collisionMargin; } set { CollisionMargin = value; } }
        ...
        public CapsuleShape(Fix32 length, Fix32 radius)
        {
        ...
        public CapsuleShape(Fix32 length, ConvexShapeDescription description)
        {
            halfLength = length .Mul (F64.C0p5);
        ...
        public static ConvexShapeDescription ComputeDescription(Fix32 length, Fix32 radius)
        {
            ConvexShapeDescription description;
			Fix32 radiusSquared = radius.Mul(radius);

            description.EntityShapeVolume.Volume = MathHelper.Pi .Mul (radiusSquared) .Mul (length) .Add (F64.FourThirdsTimesPI .Mul (radiusSquared) .Mul (radius));

            description.EntityShapeVolume.VolumeDistribution = new Matrix3x3();
            Fix32 effectiveLength = length .Add (radius .Div (F64.C2)); //This is a cylindrical inertia tensor. Approximate.
            Fix32 diagValue = F64.C0p0833333333 .Mul (effectiveLength) .Mul (effectiveLength) .Add (F64.C0p25 .Mul (radiusSquared));
        ...
        {
            if (direction.Y > Fix32.Zero)
                extremePoint = new Vector3(Fix32.Zero, halfLength, Fix32.Zero);
            else if (direction.Y < Fix32.Zero)
                extremePoint = new Vector3(Fix32.Zero, halfLength.Neg(), Fix32.Zero);
            else
        ...
        public override bool RayTest(ref Ray ray, ref RigidTransform transform, Fix32 maximumLength, out RayHit hit)
        {
            ...
            Fix32 backwardsDistance = collisionMargin .Mul ((F64.C1.Sub(squaredDistance.Div(collisionMargin.Mul(collisionMargin)))).Sqrt());
        ...
        upperSphereTest:
            ...
            var spherePosition = new Vector3(Fix32.Zero, halfLength, Fix32.Zero);
        ...
        lowerSphereTest:
            ...
            //We're above the capsule, so cast a ray against the upper sphere.
            //We don't have to worry about it hitting the bottom of the sphere since it would have hit the cylinder portion first.
            spherePosition = new Vector3(Fix32.Zero, halfLength.Neg(), Fix32.Zero);
            if (Toolbox.RayCastSphere(ref localRay, ref spherePosition, collisionMargin, maximumLength, out hit))
            {
                //Pull the hit into world space.
                Quaternion.Transform(ref hit.Normal, ref transform.Orientation, out hit.Normal);
