...
namespace BEPUphysics.Paths
{
    ...
    public abstract class SpeedControlledCurve<TValue> : Path<TValue>
    {
        ...
        public abstract Fix GetSpeedAtCurveTime(Fix time);
        ...
        public Fix GetInnerTime(Fix time)
        {
            if (Curve == null)
            ...
            Fix curveTime = (time.Sub(samples[indexMin].Wrapped)).Div((samples[indexMin + 1].Wrapped.Sub(samples[indexMin].Wrapped)));
        ...
        public void Evaluate(Fix time, out TValue value, out Fix innerTime)
        {
        ...
        /// <param name="time">Time to evaluate the curve at.</param>
        /// <param name="value">Value of the curve at the given time.</param>
        public override void Evaluate(Fix time, out TValue value)
        {
        ...
        public override void GetPathBoundsInformation(out Fix startingTime, out Fix endingTime)
        {
            if (samples.Count > 0)
            {
                startingTime = F64.C0;
        ...
        public void ResampleCurve()
        {
            ...
            Fix inverseSampleCount = F64.C1.Div((SamplesPerInterval + 1).ToFix());

            Fix speed = GetSpeedAtCurveTime(Curve.ControlPoints[minIndex].Time);
            Fix previousSpeed = speed;
...
curveTime.Add(curveTimePerSample);
        ...
        protected abstract Fix GetDistance(TValue start, TValue end);
    }
}