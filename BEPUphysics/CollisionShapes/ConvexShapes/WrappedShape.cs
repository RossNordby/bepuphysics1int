...
boundingBox.Max.X.Add(collisionMargin);
        ...
        public Fix ComputeMaximumRadius()
        {
            ...
            Fix maxRadius = F64.C0;
            for (int i = 0; i < shapes.Count; i++)
            {
                Fix radius = shapes.WrappedList.Elements[i].CollisionShape.MaximumRadius.Add(shapes.WrappedList.Elements[i].Transform.Position.Length());
                if (radius > maxRadius)
                    maxRadius = radius;
            }
        ...
        public Fix ComputeMinimumRadius()
        {
            //Could also use the tetrahedron approximation approach.
            Fix minRadius = F64.C0;
            for (int i = 0; i < shapes.Count; i++)
            {
                Fix radius = shapes.WrappedList.Elements[i].CollisionShape.MinimumRadius;
                if (radius < minRadius)
                    minRadius = radius;
            }
