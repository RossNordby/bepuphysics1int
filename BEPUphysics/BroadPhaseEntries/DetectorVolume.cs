    ...
    public class DetectorVolume : BroadPhaseEntry, ISpaceObject, IDeferredEventCreator
    {
        ...
        internal bool IsPointContained(ref Vector3 point, RawList<int> triangles)
        {
            ...
            for (int i = 0; i < triangles.Count; i++)
            {
                ...
                RayHit hit;
                bool hitClockwise;
                if (Toolbox.FindRayTriangleIntersection(ref ray, Fix32.MaxValue, ref a, ref b, ref c, out hitClockwise, out hit))
                {
                    if (hit.T < minimumT)
                    {
                        minimumT = hit.T;
            ...
            return minimumT < Fix32.MaxValue && minimumIsClockwise == innerFacingIsClockwise;
        }
