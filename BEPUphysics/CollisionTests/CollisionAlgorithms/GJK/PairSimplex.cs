...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms.GJK
{
    ...
    public struct PairSimplex
    {
        ...
        /// Defaults to 1e-8f.
        ///</summary>
        public static Fix ProgressionEpsilon = 1e-4m.ToFix();
        /// <summary>
        /// The baseline amount that an iteration must converge with its distance to avoid exiting.
        /// Defaults to 1e-7f.
        /// </summary>
        public static Fix DistanceConvergenceEpsilon = 1e-3m.ToFix();
        ...
        private PairSimplex(ref RigidTransform localTransformB)
        {
            ...
            previousDistanceToClosest = Fix.MaxValue;
        ...
        public void GetPointOnTriangleClosestToOrigin(out Vector3 point)
        {
            ...
            Fix vc = (AdotAB.Mul(BdotAC)).Sub(BdotAB.Mul(AdotAC));
            ...
            //AdotAB = -AdotAB;
            //AdotAC = -AdotAC;
            Fix vb = (CdotAB.Mul(AdotAC)).Sub(AdotAB.Mul(CdotAC));
            ...
            //BdotAB = -BdotAB;
            //BdotAC = -BdotAC;
            Fix va = (BdotAB.Mul(CdotAC)).Sub(CdotAB.Mul(BdotAC));
            Fix d3d4;
            Fix d6d5;
            if (va <= F64.C0 && (d3d4 = BdotAC.Sub(BdotAB)) > F64.C0 && (d6d5 = CdotAB.Sub(CdotAC)) > F64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
                //Throw away A.  C->A.
        ...
        public bool GetPointOnTetrahedronClosestToOrigin(out Vector3 point)
        {
            ...
            point = new Vector3();
            Fix minimumDistance = Fix.MaxValue;


            PairSimplex candidate;
            Fix candidateDistance;
            ...
            if (minimumDistance < Fix.MaxValue)
            {
                                                   ...
                                                   ref Vector3 otherPoint, out PairSimplex simplex, out Vector3 point)
        {
            ...
            if (AdotN.Mul(ADdotN) >= (Toolbox.Epsilon.Neg()).Mul(errorTolerance))
            {
                //The point we are comparing against the triangle is 0,0,0, so instead of storing an "A->P" vector,
                //just use -A.
                //Same for B->, C->P...
                ...
                Fix denom = ((va.Add(vb)).Add(vc));
                simplex.W = vc.Div(denom);
                simplex.V = vb.Div(denom);
        ...
        internal Fix errorTolerance;
        ///<summary>
        /// Gets the error tolerance of the simplex.
        ///</summary>
        public Fix ErrorTolerance
        {
            get
            {
        ...
        public bool GetNewSimplexPoint(ConvexShape shapeA, ConvexShape shapeB, int iterationCount, ref Vector3 closestPoint)
        {
            ...
            Fix distanceToClosest = closestPoint.LengthSquared();

            Fix progression = dotS.Add(distanceToClosest);
