...
namespace BEPUphysics.CollisionTests.CollisionAlgorithms
{
    ...
    public class TriangleConvexPairTester : TrianglePairTester
    {
        ...
        private bool DoPlaneTest(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Fix64 t = (dotA.Sub(dotE)).Div(reverseNormal.LengthSquared());
            ...
            Fix64 marginSum = triangle.collisionMargin.Add(convex.collisionMargin);
            ...
            if (t <= F64.C0 || distanceSquared < marginSum.Mul(marginSum))
            {
                ...
                Fix64 distance = normalLength.Mul(t);
        ...
        private bool DoExternalNear(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            Fix64 margin = convex.collisionMargin.Add(triangle.collisionMargin);
        ...
        private bool DoDeepContact(TriangleShape triangle, out TinyStructList<ContactData> contactList)
        {
            ...
            if (MPRToolbox.AreLocalShapesOverlapping(convex, triangle, ref center, ref Toolbox.RigidIdentity))
            {
                ...
                else
                {
                    ...
                    Vector3.Divide(ref triangleNormal, Fix64Ext.Sqrt(lengthSquared), out triangleNormal);
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > F64.C0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < F64.C0))
                    {
                        ...
                        if (length > Toolbox.Epsilon)
                        {
                            ...
                            Vector3.Divide(ref contact.Normal, Fix64Ext.Sqrt(length), out contact.Normal);
                    ...
                    if ((triangle.sidedness == TriangleSidedness.Clockwise && dot > F64.C0) || (triangle.sidedness == TriangleSidedness.Counterclockwise && dot < F64.C0))
                    {
                        ...
                        {
                            //Renormalize the corrected normal.
                            Vector3.Divide(ref candidateNormal, Fix64Ext.Sqrt(length), out candidateNormal);
        ...
        private bool TryInnerSphereContact(TriangleShape triangle, out ContactData contact)
        {
            ...
            Fix64 minimumRadius = convex.MinimumRadius.Mul((MotionSettings.CoreShapeScaling.Add(F64.C0p01)));
            if (length < minimumRadius.Mul(minimumRadius))
            {
                ...
                else
                {
                    ...
                    {
                        Vector3.Divide(ref triangleNormal, Fix64Ext.Sqrt(normalLength), out triangleNormal);
                        if (triangle.sidedness == TriangleSidedness.Clockwise)
        ...
        private VoronoiRegion GetVoronoiRegion(TriangleShape triangle, ref Vector3 p)
        {
            ...
            Fix64 vc = (APdotAB.Mul(BPdotAC)).Sub(BPdotAB.Mul(APdotAC));
            ...
            Fix64 vb = (CPdotAB.Mul(APdotAC)).Sub(APdotAB.Mul(CPdotAC));
            ...
            Fix64 va = (BPdotAB.Mul(CPdotAC)).Sub(CPdotAB.Mul(BPdotAC));
            if (va <= F64.C0 && (BPdotAC.Sub(BPdotAB)) > F64.C0 && (CPdotAB.Sub(CPdotAC)) > F64.C0)//Note > instead of >= and < instead of <=; prevents bad denominator
            {
        ...
        public override VoronoiRegion GetRegion(TriangleShape triangle, ref ContactData contact)
        {
            ...
            if (dotA > dotB && dotA > dotC)
            {
                ...
                if (dotB > dotC)
                {
                    ...
                    if (Fix64Ext.Abs(dotA.Sub(dotC)) < faceEpsilon)
                    {
                ...
                else
                {
                    ...
                    if (Fix64Ext.Abs(dotA.Sub(dotB)) < faceEpsilon)
                    {
                        //The normal is basically a face normal.  This can happen at the edges occasionally.
                        return VoronoiRegion.ABC;
                    }
            ...
            else if (dotB > dotC)
            {
                ...
                {
                    //C is second most extreme.
                    if (Fix64Ext.Abs(dotB.Sub(dotA)) < faceEpsilon)
                    {
                        //The normal is basically a face normal.  This can happen at the edges occasionally.
                        return VoronoiRegion.ABC;
                    }
                ...
                else
                {
                    ...
                    if (Fix64Ext.Abs(dotB.Sub(dotC)) < faceEpsilon)
                    {
                        //The normal is basically a face normal.  This can happen at the edges occasionally.
                        return VoronoiRegion.ABC;
                    }
            ...
            else
            {
                ...
                {
                    //A is second most extreme.
                    if (Fix64Ext.Abs(dotC.Sub(dotB)) < faceEpsilon)
                    {
                        //The normal is basically a face normal.  This can happen at the edges occasionally.
                        return VoronoiRegion.ABC;
                    }
                ...
                {
                    //B is second most extreme.
                    if (Fix64Ext.Abs(dotC.Sub(dotA)) < faceEpsilon)
                    {
                        //The normal is basically a face normal.  This can happen at the edges occasionally.
                        return VoronoiRegion.ABC;
                    }
