...
namespace BEPUphysicsDemos
{
    public class CharacterCameraControlScheme : CameraControlScheme
    {
            ...
            : base(camera, game)
        {
            ...
            StandingCameraOffset = 0.7m.ToFix();
            CrouchingCameraOffset = 0.4m.ToFix();
        ...
        public override void Update(Fix dt)
        {
            ...
            if (UseCameraSmoothing)
            {
				...
				Fix verticalError = Vector3.Dot(error, up);
                ...
                {
                    Camera.Position -= up * (Character.StepManager.MaximumStepHeight.Sub(verticalError));
                    verticalError = Character.StepManager.MaximumStepHeight;
                }
                else if (verticalError < Character.StepManager.MaximumStepHeight.Neg())
                {
                    Camera.Position -= up * (Character.StepManager.MaximumStepHeight.Neg().Sub(verticalError));
                    verticalError = Character.StepManager.MaximumStepHeight.Neg();
                }
				//Clamp the horizontal distance too.
				Fix horizontalErrorLength = horizontalError.LengthSquared();
				Fix margin = Character.Body.CollisionInformation.Shape.CollisionMargin;
				...
				Fix errorCorrectionFactor = 1.ToFix().Sub(Fix32Ext.Pow(.000000001m.ToFix(), dt));
