...
namespace AsyncUsageAnalyzers.Helpers {
	...
	internal partial class CustomBatchFixAllProvider : FixAllProvider {
		const string fixAllTitle = "Apply fixes in Batch (Custom) per document.";
		protected CustomBatchFixAllProvider() {
		}
		
		public static readonly FixAllProvider Instance = new CustomBatchFixAllProvider();
		...
		public virtual async Task<CodeAction> TryGetMergedFixAsync(IEnumerable<CodeAction> batchOfFixes, FixAllContext fixAllContext) {
			...
			if (newSolution != null && newSolution != solution) {
				return CodeAction.Create(fixAllTitle, cancellationToken => Task.FromResult(newSolution));
			}
